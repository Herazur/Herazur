...
const avatarFor = (p) => {
  ...
  // .gif ise ve GIF kullanımı yoksa ilk kareyi göster
...
export default function FollowingList({ userId, refreshKey }) {
  const { getFollowing } = useData();
  ...
  const list = useMemo(() => {
    ...
    return following
      ...
      .map((p, index) => ({
        id: p?.user_id ?? p?.id ?? p?.username ?? `following-${index}`,
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="mt-4 text-lg font-semibold">Error Loading Following</h3>
  ...
  if (list.length === 0) {
    return (
      <div className="text-center py-12">
        <UserCheck className="mx-auto h-12 w-12 text-muted-foreground" />
        <h3 className="mt-4 text-lg font-semibold">Not Following Anyone</h3>
        <p className="mt-1 text-sm text-muted-foreground">
  ...
  return (
    <div className="mt-8">
      <h2 className="text-2xl font-bold mb-4">Following ({list.length})</h2>
