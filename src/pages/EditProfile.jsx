...
const SESSION_STORAGE_KEY = 'editProfileForm';

const EditProfile = () => {
  const { user, profile, loading: authLoading, refreshProfile } = useAuth();
  const { tags: allTags, loading: dataLoading, updateProfile } = useData();
  ...
  }, [profile, authLoading, navigate, reset]);
  ...
  const addSocialLink = () => {
    if (socialLinks.length < 5) {
      setValue('links', [...socialLinks, { platform: '', url: '' }], { shouldDirty: true });
    } else {
  ...
  return (
    <>
      <Seo
        ...
        description="Update your Herazur profile details."
        noindex
      />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="text-center mb-8">
              <h1 className="text-4xl font-bold gradient-text">Edit Your Profile</h1>
              <p className="text-muted-foreground mt-2">Keep your profile fresh and up to date.</p>
              {canUseAnimatedGif && (
            ...
            <div className="flex justify-end">
              <Button type="submit" size="lg" disabled={isSubmitting || (!isDirty && !avatarFile && !bannerFile)}>
